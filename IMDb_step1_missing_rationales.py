...
args = {
    ...
    'new_save_dir': './MR_results/IMDb_step0_mr_trainer',
}
...
def construct_training_eval(args,random_seed,num_per_example=7): 
    ...
    ## reading false rationales augmented data
    ...
    ## magnify with missing rationales
    train_texts = []
    train_labels = []
    ...
    for key in train_keys:
        ...
        threshold = num_per_example

        ## if no missing rationales, using false-rationales
        if len(missing_augmented_data[key]['candidates']) == 0:
    
            
            train_texts = train_texts + [IMDb_data[key]['text']]*num_per_example
            train_labels = train_labels + [label]*num_per_example


        # if num of missing rationales is less then num of augmented examples, using all missing ratioanles plus some false-rationales
            ...
            num_for_rest =int(num_per_example - len(missing_candidates))
            
        
            train_texts = train_texts + [IMDb_data[key]['text']]*num_for_rest
            train_labels = train_labels + [label]*num_for_rest

        # all using all missing rationales
        elif len(missing_augmented_data[key]['candidates'])>threshold:
